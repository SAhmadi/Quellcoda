<template>
  <div class="icon-bar">
    <div type="button" class="btn btn-icon" @click="runProgram">
    ...
    <div type="button" class="btn btn-icon" @click="testProgram">
    ...
    <div type="button" class="btn btn-icon" @click="generateGradleProject">
  ...
  methods: {
    async runProgram() {
      ...
      for (let i = 0; i < models.length; i++) {
        zip.file('src/' + models[i].path + models[i].name, models[i].value);
      }

      await zip.generateAsync({ type: 'blob', compression: "DEFLATE" })
        .then(blob => {
          let fileObj = new File([blob], 'src.zip', {type: 'zip'});
          let formData = new FormData();
          formData.append('file', fileObj);

          axios
            .post(
              'https://ba-serverless-testing-t6e6p4w6oa-ew.a.run.app/run/zip',
              formData,
              {
                headers: {
            ...
            .then(response => {
              ...
              console.log(this.$store.getters.consoleText);
            })
    ...
    /*async runProgram() {
      ...
      let models = this.$store.getters.models;
      let files = this.$store.getters.fileTreeChildren;
      let newFileTreeChildren = [];
      ...
      while (true) {
        ...
        if (file && file.children) {
          currZipPath.push(file.name + '/');
          //currZipPath = currZipPath + file.name + '/';
        ...
        // Find file model in model list
        for (let i = 0; i < models.length; i++) {
          if (file && file.id === models[i].id) {
            let flattenedPath = currZipPath.join('');
            console.log(flattenedPath + file.name);
            zip.file(flattenedPath + file.name, file.model.value);
        ...
        if (currIndex < currFiles.length) {
          ...
          // Remove top folder from zip path
          currZipPath.pop();
    ...
    generateGradleProject() {
      console.log('Generating gradle project...');
      let gradleFileTree = {
        children: [
          {
            id: 1,
                                ...
                                children: [
                                  {
                                    ...
                                    name: 'MainTest.java',
                                    model: {
                                      value: '// This is your main test file...',
                                      language: 'java',
                                    },
                                  },
                                ],
      ...
      this.$store.dispatch('updateFileTree', gradleFileTree.children);

      // Create models for new files
      this.$store.dispatch('updateAllModels', []);
      this.createModels(gradleFileTree.children);

      this.$store.dispatch('updateActiveNode', gradleFileTree.children[0]);
